...
impl<K: Ord, V, const N: usize> FromIterator<(K, V)> for SGTree<K, V, N> {
...
impl<'a, K: Ord, V, const N: usize> IntoIterator for &'a SGTree<K, V, N> {
    type Item = (&'a K, &'a V);
...
// Consuming iterator
impl<K: Ord, V, const N: usize> IntoIterator for SGTree<K, V, N> {
    type Item = (K, V);
